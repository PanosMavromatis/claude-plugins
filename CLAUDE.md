...
**Paths written into permanent records decay; identities do not.** `/smart-merge` puts a `Plan:` pointer in each PR body, and PR bodies are effectively immutable. Because plan locations are deliberately free, a path pointer breaks the moment a plan is filed — the first sweep silently invalidated seven of them. Anywhere this system writes a plan reference into something that cannot later be edited, write the **name** and let the reader resolve it the way the commands do. Same rule as resolution, applied to records instead of lookups.
