...
- `AGENTS.md` and `AGENTS.override.md` are **generated artifacts** built from the `docs/agents/` sources by `dev/build-agents-md.sh`.
- A PreToolUse hook at `.claude/hooks/protect-agent-docs.py` blocks `Write|Edit|MultiEdit` against `CLAUDE.md`, `AGENTS.md`, and `AGENTS.override.md`. Bash redirects pass through, which is how the build script can write the generated files.

`/agents-docs-init` migrates a legacy single-file `CLAUDE.md` into this split layout. `/agents-docs-codex-init` generates the Codex sidecar. `/agents-docs-update` edits the `docs/agents/*.md` sources (never the generated artifacts) and re-runs the build script when `core.md` or `codex.md` changed.

When editing the doc-related commands here, do not add logic that writes to `CLAUDE.md`, `AGENTS.md`, or `AGENTS.override.md` directly — it would (correctly) be blocked by the consumer's hook. Always edit the `docs/agents/*.md` sources and let the build script regenerate the rest.
