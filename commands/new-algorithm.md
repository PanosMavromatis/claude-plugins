...
4. Hand off to the `algorithm-formalize` skill to guide the Phase 0 (pseudocode) implementation. **Use the `Skill` tool with `skill: "dp-compile:algorithm-formalize"`** — do NOT use `Read` on `skills/algorithm-formalize/SKILL.md`. Reading the file just dumps its contents into context; only the `Skill` tool activates the skill through the harness so it runs with its own references and examples. If the `Skill` tool reports the skill is not available, stop and tell the user: "The `algorithm-formalize` skill is missing — check that the plugin loaded correctly."

5. When Phase 0 is complete (formalization reviewed and approved), inform the user: "Phase 0 is done. When you're ready, run `/dp-compile:next-phase <name>` to start the Python prototype implementation."
