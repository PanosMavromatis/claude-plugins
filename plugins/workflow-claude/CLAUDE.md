...
- **`/open-revision <label>`** opens a revision and writes its `## Subgoals — revision <label>` section into the master plan. The user supplies the label and the preamble; the command invents neither. See "Revisions have a full lifecycle" below.
...
- **`/close-revision <label>`** cuts a finished revision's section out of the master plan into `docs/plan/<label>/_DO.md`, leaving a pointer under `## Closed revisions`. Run `/file-plans` first. The user decides a revision is finished; the command refuses while items are open. See "Closing a revision" below.
...
**Revisions have a full lifecycle and are identified by a label.** `/open-revision <label>` creates the `## Subgoals — revision <label>` section; `/file-plans` gathers the revision's branch plans into `docs/plan/<label>/`; `/close-revision <label>` archives it. The label — e.g. `04-revision-lifecycle` — is the identity, naming heading and directory alike, which is the identity-vs-location rule applied one level above branch plans. A leading ordinal is a sorting convention *inside* the label: nothing counts revisions, checks the ordinal, or requires them to be consecutive, so a project may skip numbers or use dates instead.

`/open-revision` deliberately does **not** create the directory — an empty directory is untracked by git, so it would either be lost or need a pointless placeholder. The directory appears when `/file-plans` files the first plan into it, which is why that command must `mkdir -p` its destination: `git mv` fails outright on a missing destination directory, and every revision's first filing hits that case. The bug was latent for two revisions only because `rev-2/` and `rev-3/` had been created by hand before `/file-plans` existed.

**Closing a revision extracts it from the master plan.** `/close-revision <label>` (backed by `scripts/close-revision.sh`) cuts the section out of `docs/plan/DO.md` and writes it to `docs/plan/<label>/_DO.md`, leaving a one-line pointer under `## Closed revisions`. Without this the master plan accumulates every subgoal the project has ever completed, each with its `> **Done:**` annotation, and never sheds any of it.
...
The extraction is a **cut**, not a copy — a section surviving in both files defeats the purpose and lets the copies drift — and the archive is **verbatim**, since summarizing a closed revision destroys the record the two-tier convention exists to keep. Run `/file-plans` before `/close-revision`, or the revision's directory holds an archive describing plans still sitting flat elsewhere. Note that the *last* branch of a revision can never be filed before that revision closes — its plan is stamped `merged` only at its own merge — so file between the merge stamp and the close, or accept that one plan waits for the next sweep.
...
**The status stamp is a contract between five commands.** `/new-branch` writes `**Status**: active`; `/smart-merge` rewrites it to `**Status**: merged — PR #<n> — <date>`; `/step` and `/hitl-step` read it to filter rung 4; and `/file-plans` reads it to decide which plans are finished enough to file. The count grew when `/file-plans` landed and was not updated then — this sentence said "three" while enumerating four, which is the failure mode of counting things in prose. Prefer naming the readers over counting them. A plan counts as merged **only** if a `**Status**:` line's value begins with `merged` — everything else, including no stamp at all, counts as active. That asymmetry is deliberate: plans merged outside `/smart-merge` never get stamped, and a stale option in a list is cheaper than a hidden live plan. When changing the format, change every one of them together.
...
- **Script-backed commands: the script proposes, the command writes.** Four commands are backed by a bundled script — `/agents-docs-build`, `/open-revision`, `/file-plans`, `/close-revision`. For the three plan-lifecycle ones the split is strict: `scripts/*.sh` are **read-only** and print a proposal, and every write, confirmation and commit lives in the command. That keeps the scripts safe to run directly, in a hook, or in CI, and keeps destructive verbs (`git mv`, deletions) out of `allowed-tools` where the harness prompt can act as a second gate. When adding logic, ask which half it belongs in — a script that starts writing loses the property that makes it callable from anywhere.

  Two portability traps these scripts have already hit, both of which fail **silently**: BSD `grep` does not support `\|` alternation in a basic regex and matches nothing rather than erroring, so match on exact strings with `awk` rather than assembling alternations; and BSD `wc -l` pads its output with spaces, which corrupts both arithmetic and any report a human reads before approving a write. Test on macOS *and* assume a consumer is on Linux.
