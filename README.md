...
| `/smart-merge`  | Reads the branch doc and plan to draft PR title and body, warns on unfinished plan items, deletes the doc, stamps the plan `merged` and closes the master-plan item in one pre-merge commit, then merges via `gh pr merge`. Walks merge-strategy choice with tradeoffs. |
