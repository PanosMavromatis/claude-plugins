...
| `/step N [path]` | Executes the next `N` unchecked items from a `DO.md` under `docs/plan/`. Plain checkbox model (`[ ]` / `[x]`), inline `> **Q:** / > **A:**` log under each item so reasoning survives `/clear` or compaction. Hard-stops at `N`. Optional path argument selects the plan file. |
| `/hitl-step N [path]` | Same loop against a `docs/plan/**/TODO.md` but with a richer marker model (`[ ] [~] [x] [!] [-]`), explicit confirmation gates on writes, and parent/subgoal state propagation. Use this when each goal needs back-and-forth with you. |
...
- **`docs/plan/**/DO.md` and `TODO.md` hold the work queue.** Plan files live under `docs/plan/` — at its root or in a sub-directory (`docs/plan/<topic>/DO.md`), so several plans can coexist. `/step` and `/hitl-step` discover the file by glob and ask which to use when more than one matches; pass a path as the second argument to pick one directly. A legacy `DO.md`/`TODO.md` at the repo root still works, with a nudge to move it.
